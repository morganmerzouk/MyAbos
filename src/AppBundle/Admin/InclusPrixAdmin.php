...
class InclusPrixAdmin extends Admin
{
    ...
    protected function configureFormFields(FormMapper $formMapper)
    {
        $optionsIcone = array(
            ...
            'attr' => array(
                'class' => 'inclusprix-miniature'
            )
        );
        
        $inclusPrix = $this->getSubject();
        
        if ($inclusPrix->getIconeWebPath()) {
            $optionsIcone['help'] = '<img src="' . $inclusPrix->getIconeWebPath() . '" class="preview-img" />';
        }
        ...
        $formMapper->add('iconeFile', 'file', $optionsIcone)
            ...
            'label' => 'Catégorie: ',
            'label_attr' => array(
                'class' => 'control-inclusprix-categorie'
            ),
